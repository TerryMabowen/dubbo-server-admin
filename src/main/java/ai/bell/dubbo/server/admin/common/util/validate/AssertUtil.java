...
import ai.bell.dubbo.server.admin.common.exception.AssertException;
import ai.bell.dubbo.server.admin.common.util.enums.EnumUtil;
...
public class AssertUtil {
    ...
    public static void assertNotNull(String value, String tips) {
        if (Objects.isNull(value)) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 字符串不能为空" : tips);
        }
    }
    ...
    public static void assertTel(String value, String tips) {
        ...
        if (!RegexUtil.isTel(value)) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 不是一个有效的手机/电话号码" : tips);
        }
    }
    ...
    public static void assertValidateEmail(String value, String tips) {
        ...
        if (!RegexUtil.isEmail(value)) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 不是一个有效的邮箱地址" : tips);
        }
    }
    ...
    public static void assertMobile(String value, String tips) {
        if (!RegexUtil.isMobile(value)) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 不是一个有效的手机号" : tips);
        }
    }
    ...
    public static void assertGT0(Number value, String tips) {
        if (value == null || value.longValue() <= 0L) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 必须大于0" : tips);
        }
    }
    ...
    public static void assertGE0(Number value, String tips) {
        if (value == null || value.doubleValue() < 0.0D) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 必须大于等于0" : tips);
        }
    }

    public static void assertNotNull(Object obj, String tips) {
        if (Objects.isNull(obj)) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 对象不存在" : tips);
        }
    }
    ...
    public static void assertEnumExist(int status, Class<? extends BaseEnumStatus> cls, String tips) {
        ...
        if (Objects.isNull(value)) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 枚举类型不存在" : tips);
        }
    }
    ...
    public static void assertEqual(Object val, Object val2, String tips) {
        if (val != null && val2 != null) {
            if (!val.equals(val2)) {
                throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 两个值不相等" : tips);
            }
        } else {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 两个值不相等" : tips);
        }
    ...
    public static void assertScope(Number start, Number end, Number value, String tips) {
        if (value == null || value.doubleValue() < start.doubleValue() || value.doubleValue() > end.doubleValue()) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 数值不再范围内" : tips);
        }
    }
    ...
    public static void assertTrue(Boolean value, String tips) {
        ...
        if (!value) {
            throw new AssertException(StrUtil.isEmpty(tips) ? "验证失败, 比如为True" : tips);
        }
    }
